...
tf.flags.DEFINE_string('output_dir', './outputs_std_bga_ens',
...
def graph(x, y, i, x_max, x_min, grad):
    ...
    for j in range(5):
        ...
        pred = tf.argmax(
            end_points_v3['Predictions'] + end_points_v4['Predictions'] +
            end_points_res_v2['Predictions'] + end_points_resnet['predictions'], 1)
    ...
    g_x = g / tf.constant(5.0)
    g_x = tf.nn.depthwise_conv2d(g_x, stack_kernel, strides=[1, 1, 1, 1], padding='SAME')
    g_x = g_x / tf.reduce_mean(tf.abs(g_x), [1, 2, 3], keep_dims=True)
    ...
    for j in range(5):
        with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
            logits_v3, end_points_v3 = inception_v3.inception_v3(
                input_diversity(x_b1), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
        with slim.arg_scope(inception_v4.inception_v4_arg_scope()):
            logits_v4, end_points_v4 = inception_v4.inception_v4(
                input_diversity(x_b1), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
        with slim.arg_scope(inception_resnet_v2.inception_resnet_v2_arg_scope()):
        ...
        cross_entropy = tf.losses.softmax_cross_entropy(one_hot, logits, label_smoothing=0.0, weights=1.0)
        cross_entropy += tf.losses.softmax_cross_entropy(one_hot, auxlogits, label_smoothing=0.0, weights=0.4)
        noise = tf.gradients(cross_entropy, x_b1)[0]
        g = g + noise
        x_b1 = x_b1 / 2.0

    g_x_b1 = g / tf.constant(5.0)
    g_x_b1 = tf.nn.depthwise_conv2d(g_x_b1, stack_kernel, strides=[1, 1, 1, 1], padding='SAME')
    g_x_b1 = g_x_b1 / tf.reduce_mean(tf.abs(g_x_b1), [1, 2, 3], keep_dims=True)

    g = tf.zeros(shape=batch_shape)
    for j in range(5):
        with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
            logits_v3, end_points_v3 = inception_v3.inception_v3(
                input_diversity(x_p1), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
        with slim.arg_scope(inception_v4.inception_v4_arg_scope()):
            logits_v4, end_points_v4 = inception_v4.inception_v4(
                input_diversity(x_p1), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
        with slim.arg_scope(inception_resnet_v2.inception_resnet_v2_arg_scope()):
        ...
        cross_entropy = tf.losses.softmax_cross_entropy(one_hot, logits, label_smoothing=0.0, weights=1.0)
        cross_entropy += tf.losses.softmax_cross_entropy(one_hot, auxlogits, label_smoothing=0.0, weights=0.4)
        noise = tf.gradients(cross_entropy, x_p1)[0]
        g = g + noise
        x_p1 = x_p1 / 2.0

    g_x_p1 = g / tf.constant(5.0)
    g_x_p1 = tf.nn.depthwise_conv2d(g_x_p1, stack_kernel, strides=[1, 1, 1, 1], padding='SAME')
    g_x_p1 = g_x_p1 / tf.reduce_mean(tf.abs(g_x_p1), [1, 2, 3], keep_dims=True)
